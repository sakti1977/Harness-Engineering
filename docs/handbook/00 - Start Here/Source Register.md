...
The original notes came from earlier shared material whose authorship and publication cannot be recovered here. [Source Coverage Index](Source%20Coverage%20Index.md) preserves that thematic mapping. Sources below support the new explanations; they do not retroactively establish the origin of the five-layer vocabulary or clinic stories.
...
- **Executed local result:** a command and observed output recorded in [Vault Update Record](Vault%20Update%20Record.md).

All sources were accessed during this task on 2026-09-26. Repository main links are moving references unless pinned. Publication dates are recorded only where verified. Star counts live in the dated research inventory and are not quality scores. Use [Source Note Template](../Templates/Source%20Note%20Template.md) for deeper source cards.
...
S7 (LangChain’s Deep Agents benchmark account) remains in [engineering_sources](../research_notes/Harness%20knowledge%20base%20expansion/engineering_sources.md); this expansion does not reuse its numerical uplift as a local claim.
...
[Repository Pattern Atlas](../13%20-%20Research/Repository%20Pattern%20Atlas.md) maps the 13 repositories selected in [repositories](../research_notes/Harness%20knowledge%20base%20expansion/repositories.md) to learning questions and local notes. Review depth varies. No upstream benchmark or deployment was reproduced during the source survey.

## Review triggers

Revisit claims when a cited page changes, a model/runtime is upgraded, an adapter fails, a specification changes or a reader supplies conflicting evidence. Mechanical link validation and semantic source review are separate activities. See [Knowledge Base Maintenance](../02%20-%20Reach/Knowledge%20Base%20Maintenance.md).
