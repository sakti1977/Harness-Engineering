...
Use [[Experiment Template]] to specify two identical local runs. Change only a resource limit, then inspect whether the failure occurred before the agent acted or during task execution. Preserve both outcomes; do not quietly discard inconvenient timeouts.
